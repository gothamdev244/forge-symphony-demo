---
name: orbit-create-beads
description: "Convert PRDs to beads for orbit execution. Creates an epic with child beads for each user story. Use when you have a PRD and want to use orbit with beads as the task source. Triggers on: create beads, convert prd to beads, beads for ralph, ralph beads."
---

# Orbit - Create Beads

Converts PRDs to beads (epic + child tasks) for orbit autonomous execution.

> **Note:** This skill is bundled with orbit's Beads tracker plugin. Future tracker plugins (Linear, GitHub Issues, etc.) will bundle their own task creation skills.
...
5. Output ready for `orbit run --tracker beads`
...
bd dep add orbit-002 orbit-001  # US-002 depends on US-001
bd dep add orbit-003 orbit-002  # US-003 depends on US-002
```
...
bd dep add orbit-002 orbit-001
...
bd dep add orbit-003 orbit-002
```
...
orbit run --tracker beads --epic orbit-abc

# Or let it pick the best task automatically
orbit run --tracker beads
```
