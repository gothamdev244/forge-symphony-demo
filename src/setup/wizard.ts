/**
 * ABOUTME: Interactive setup wizard for Orbit TUI.
 * Guides users through initial configuration when no .orbit/config.toml exists.
...
/**
 * Save configuration to .orbit/config.toml
 */
async function saveConfig(
  ...
  // Add header comment
  const content = `# Orbit TUI Configuration
# Generated by setup wizard
# See: orbit config help
...
export async function runSetupWizard(
  ...
  try {
    ...
    if (!isInteractiveTerminal()) {
      return {
        ...
        error:
          ...
          'configuration file manually at .orbit/config.toml',
      };
    }
    ...
    console.log();
    console.log('╔════════════════════════════════════════════════════════════╗');
    console.log('║                   Orbit TUI Setup Wizard                    ║');
    console.log('╚════════════════════════════════════════════════════════════╝');
    console.log();
    printInfo('This wizard will help you configure Orbit TUI for your project.');
    ...
    const selectedTracker = await promptSelect(
      ...
      trackerChoices,
      {
        ...
        help: 'Orbit will use this tracker to manage tasks.',
      }
    );
    ...
    if (isBeadsTracker) {
      ...
      if (selectedPlugin?.available) {
        console.log();
        printInfo('Beads tracker tip: When running Orbit, specify an epic:');
        console.log('  orbit run --epic <epic-id>');
        console.log();
        printInfo('Or omit --epic to get an interactive epic selection list.');
        console.log();
        printInfo('Have a markdown PRD? Convert it to Beads issues:');
        console.log('  orbit convert --to beads --input ./prd.md');
      }
    }
    ...
    printInfo('Orbit supports multiple AI coding agents.');
    if (defaultAgent) {
    ...
    if (!skillsPaths) {
      ...
      if (bundledSkills.length > 0) {
        ...
        printInfo('Orbit TUI includes AI skills that enhance agent capabilities.');
    ...
    if (preflightResult.success) {
      ...
      printInfo('Run "orbit doctor" to diagnose issues.');
      console.log();
    }

    // Show tracker-specific instructions
    if (selectedTracker === 'json') {
      printInfo('You can now run Orbit TUI with:');
      console.log();
      console.log('  Create a PRD and tasks:        orbit create-prd');
      console.log('  Run Orbit on existing tasks:   orbit run --prd <path-to-prd.json>');
    } else if (selectedTracker === 'beads' || selectedTracker === 'beads-bv' || selectedTracker === 'beads-rust') {
      printInfo('You can now run Orbit TUI with:');
      console.log();
      console.log('  orbit run                  # Interactive epic selection');
      console.log('  orbit run --epic <id>      # Run with a specific epic');
      console.log();
      ...
      printInfo('You can now run Orbit TUI with:');
      console.log();
      console.log('  orbit run');
    }
    console.log();
    printInfo('Or edit the configuration with:');
    console.log();
    console.log('  orbit config show');
    console.log();
...
export async function checkAndRunSetup(
  ...
  if (options.skipSetup) {
    ...
    printInfo('No configuration found. Run "orbit setup" to create one.');
    return null;
  }
